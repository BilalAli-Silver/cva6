...
# ELF runner for CVA6 cv32a6_imac_sv32 (veri-testharness). ACT run_cmd.txt invokes:
#   run-cv32a6_imac_sv32.sh {debug:--trace __TRACEFILE__} --elf <elf>
#
...
SPIKE_ISA="${SPIKE_ISA:-rv32imac_zicsr_zifencei_zba_zbb_zbc_zbs_zbkb_zbkx_zkne_zknd_zknh}"
...
if [[ -n "${trace_log}" ]]; then
  shopt -s nullglob
  rvfi_dasm_files=(trace_rvfi_hart_*.dasm)
  if [[ ${#rvfi_dasm_files[@]} -gt 0 ]]; then
    cat "${rvfi_dasm_files[@]}" | "${SPIKE_PATH}/spike-dasm" --isa="${SPIKE_ISA}" > "${trace_log}"
    rm -f "${rvfi_dasm_files[@]}"
  else
    echo "$(basename "$0"): warning: trace_rvfi_hart_*.dasm not found; no instruction trace for ${trace_log}" >&2
  fi
fi
